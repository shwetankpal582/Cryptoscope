...
const NewsPage = async () => {
    async function fetchData() {
        let data: any = []
        const response: any = await fetch(`https://newsapi.org/v2/everything?q=cryptocurrency&apiKey=${process.env.newsApiKey}`).then((res) => res.json())
        for (let i = 0; i < 5; i++) {
            data.push(response.articles[i]);
        }
        return data
    }
