...
import { CoinData, CoinTimeSeries } from '@/app/interface';
const MarketStat = async () => {

    async function fetchData() {
        let data: CoinTimeSeries[] = []
        ...
        for (let i = 0; i < snapshot.docs.length; i++) {
            ...
            if (!snapshot.docs[i].data().historical_data?.["Error Message"])
                data.push(snapshot.docs[i].data().historical_data as CoinTimeSeries);
        }
        return data
    }
    let data: CoinTimeSeries[] = await fetchData()
    return (
